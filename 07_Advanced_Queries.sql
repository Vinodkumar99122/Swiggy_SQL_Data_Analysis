...
- Moving average of restaurants ratings
...
--1.CTEs - Top 5 highest rated restaurants
WITH TopRated AS (
...
--2. CTEs with aggregate- average price per city
WITH CityAvg AS (
...
--3. Window function RANK restaurant by rating per city
SELECT Restaurant, City, TRY_CAST(Avg_Ratings AS FLOAT) AS Rating,
...
SELECT Restaurant, City, TRY_CAST(Total_Ratings AS INT) AS Total_Rating,
	SUM(TRY_CAST(Total_Ratings AS INT)) OVER(PARTITION BY City ORDER BY Restaurant ) AS Cumulative_Rank
FROM Swiggy_Data;

--6. Window function - moving average of ratings per restaurant
SELECT Restaurant, City, TRY_CAST(Avg_Ratings AS FLOAT) AS Avg_Rating,
	AVG(TRY_CAST(Total_Ratings AS FLOAT)) OVER(PARTITION BY City ORDER BY Restaurant ROWS BETWEEN 2 PRECEDING AND CURRENT ROW) AS Moving_Avg_Rating
FROM Swiggy_Data;

--7. CTE - Window function Top restaurant in each city
WITH Ranked AS (
		SELECT Restaurant, City, TRY_CAST(Avg_Ratings AS FLOAT) AS Rating,
			ROW_NUMBER() OVER(PARTITION BY City ORDER BY TRY_CAST(Avg_Ratings AS FLOAT)DESC) AS Row_Num
FROM Swiggy_Data
)
